...
<div id="chatbot-widget" class="fixed bottom-36 sm:bottom-24 {{ request()->is('/') ? 'right-2 md:right-4' : 'right-6 md:right-8' }} z-50 translate-y-[10px]">
    ...
    <button id="chatbot-toggle"
        ...
        <img src="{{ \App\Models\SystemSetting::get('chatbot_icon', asset('storage/chatbot_icon/SIMA nobg3.png')) }}" class="w-full h-full object-contain animate-float-bot" alt="{{ \App\Models\SystemSetting::get('chatbot_name', 'SIMA') }}">
    </button>
        ...
        <div class="bg-white px-4 py-3 flex items-center justify-between z-10 relative">
            <div class="flex items-center gap-3">
                <div class="w-8 h-8 bg-white rounded-full flex items-center justify-center shadow-sm border border-gray-100 overflow-hidden">
                    <img src="{{ \App\Models\SystemSetting::get('chatbot_profile', asset('storage/chatbot_icon/SIMA head.png')) }}" class="w-full h-full object-cover" alt="{{ \App\Models\SystemSetting::get('chatbot_name', 'SIMA') }}">
                </div>
                <span class="font-bold text-gray-800 text-base">{{ \App\Models\SystemSetting::get('chatbot_name', 'SIMA') }}</span>
            </div>
                ...
                <div class="w-48 h-48 bg-white/30 rounded-full flex items-center justify-center">
                    <div class="w-36 h-36 bg-white/70 backdrop-blur-md rounded-2xl shadow-sm flex flex-col items-center justify-center relative">
                        <div class="w-24 h-24 bg-white rounded-full flex items-center justify-center shadow-md border-4 border-blue-50 overflow-hidden">
                            <img src="{{ \App\Models\SystemSetting::get('chatbot_icon', asset('storage/chatbot_icon/SIMA nobg2.png')) }}" class="w-full h-full object-cover" alt="{{ \App\Models\SystemSetting::get('chatbot_name', 'SIMA') }}">
                        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function handleChatbotOpen() {
        if (isFirstTimeOpen) {
            ...
            setTimeout(() => {
                appendMessage('bot', 'Hai aku ' + '{{ \App\Models\SystemSetting::get('chatbot_name', 'SIMA') }}' + ' asisten yang akan membantumu. Apa yang bisa saya bantu?');
            }, 300);
        }
    }
