...
<style>
    ...
    * { font-family: 'Plus Jakarta Sans', sans-serif; }
    ...
    .avatar-ring { background: linear-gradient(135deg, #60a5fa, #818cf8); padding: 3px; border-radius: 9999px; display: inline-flex; flex-shrink: 0; }
    .avatar-inner { border-radius: 9999px; width: 80px; height: 80px; display:flex;align-items:center;justify-content:center; overflow:hidden; background: linear-gradient(135deg,#3b82f6,#6366f1); }
    ...
    .stat-tile { background:#fff;border-radius:1.25rem;padding:1.4rem;box-shadow:0 1px 3px rgba(30,58,138,0.06), 0 4px 20px rgba(30,58,138,0.06); position:relative; overflow:hidden; transition: all .3s ease; }
    ...
    .status-item { display:flex;align-items:center;justify-content:space-between;padding:14px 18px;border-radius:14px;background:#f8faff;border:1px solid #e8eeff; transition: all .2s ease; }
    ...
    .action-btn { display:flex;align-items:center;gap:10px;padding:13px 18px;border-radius:14px;background:#f0f4ff;border:1.5px solid #e0e7ff;color:#3b4fd8;font-weight:600;font-size:14px;transition:all .2s ease;width:100%; }
    .action-btn:hover { background:#e8eeff;border-color:#a5b4fc;transform:translateY(-2px);box-shadow:0 4px 12px rgba(99,102,241,0.15); }
                    ...
                    <div class="stat-tile" style="--stat-color:#10b981;">
                        <div class="flex items-start justify-between">
                            <div>
                                ...
                                <p class="text-2xl font-extrabold mt-2">{{ $withPhotoCount ?? $logbooks->where('photo_path', '!=', null)->count() }}</p>
                            </div>
                    ...
                    <div class="stat-tile" style="--stat-color:#8b5cf6;">
                        <div class="flex items-start justify-between">
                            <div>
                                ...
                                <p class="text-2xl font-extrabold mt-2">{{ $thisMonthCount ?? $logbooks->where('date', '>=', now()->startOfMonth())->count() }}</p>
                            </div>
                                    ...
                                    <tr class="hover:bg-blue-50 transition-colors duration-150">
                                        <td class="px-6 py-4 whitespace-nowrap text-center">
                                            <div class="flex items-center justify-center">
                                                <span class="text-sm font-medium text-gray-900">{{ $logbook->date->format('d/m/y') }}</span>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 text-left">
                                            <div class="text-sm text-gray-900 max-w-md">{{ Str::limit($logbook->activity, 150) }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-center">
                                            @if($logbook->photo_path)
                                                @php $filename = basename($logbook->photo_path); @endphp
                                                <img src="{{ route('intern.logbook.photo', $filename) }}" alt="Logbook photo"
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-center">
                                            <div class="flex gap-2 justify-center">
                                                ...
                                                <form action="{{ route('intern.logbook.destroy', $logbook) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus logbook ini?');">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="inline-flex items-center justify-center w-10 h-10 bg-red-100 hover:bg-red-200 rounded-lg transition-all duration-200 group" title="Hapus">
                                    ...
                                    <tr>
                                        <td colspan="4" class="px-6 py-12 text-center">
                                            <div class="flex flex-col items-center justify-center text-gray-500">
                                                ...
                                                @if ($cekaktif)
                                                    <a href="{{ route('intern.logbook.create') }}" class="mt-4 inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-lg transition-all duration-300">
